...
REM Copy CUDA runtime DLLs to target\release for execution
if defined CUDA_PATH (
    if exist "%CUDA_PATH%\bin\x64\cublas64_*.dll" (
        copy /Y "%CUDA_PATH%\bin\x64\cublas*.dll" "..\target\release\" >nul 2>&1
        copy /Y "%CUDA_PATH%\bin\x64\cudart*.dll" "..\target\release\" >nul 2>&1
        copy /Y "%CUDA_PATH%\bin\x64\cublas*.dll" "target\release\" >nul 2>&1
        copy /Y "%CUDA_PATH%\bin\x64\cudart*.dll" "target\release\" >nul 2>&1
        echo ✅ Copied CUDA runtime DLLs to target\release
    )
)
